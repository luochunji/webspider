...
public class TaobaoSpiderV2ServiceImpl implements BasePageProcessor {
     ...
     */
    private static String LIST_URL = "http://s.taobao.com/search?data-key=s&data-value={0}&ajax=true&q={1}&commend=all&ssid=s5-e&search_type=item&sourceId=tb.index&initiative_id=tbindexz_{2}&style=list&tab=all&bcoffset=1&s=0&cat=50944003";
    /**
...
/**
    private void spiderRuleData(Page page) {
        String response = page.getRawText();
        String jsonStr = response.substring(response.indexOf("(")+1,response.lastIndexOf(")"));
        JSONObject json = JSONObject.fromObject(jsonStr);
        int code = json.getInt("code");
        String itemId = getItemId(page);
        if(code == 200){
            JSONArray jsonArray = json.getJSONArray("spu");
            for(Object obj : jsonArray){
                JSONObject ruleJson = (JSONObject)obj;
                String name = ruleJson.getString("name");
                JSONArray subArray = ruleJson.getJSONArray("sub");
                ProductRule productRule = new ProductRule(name);
                for(Object subObj : subArray){
                    JSONObject subJson = (JSONObject)subObj;
                    String ruleName =  subJson.getString("name");
                    String desc = subJson.getString("desc");
                    ProductRuleDetail prd = new ProductRuleDetail(ruleName,desc);
                    productRule.addProductRuleDetail(prd);
                }
                productMap.get(itemId).addProductRule(productRule);
            }
        }
        ...
        if(200 == code){
            ...
            for(Object obj : jsonData.keySet()){
                ...
                for(Object ikey : expireDate.keySet()){
                    ...
                    ProductDetail pd = new ProductDetail(Double.valueOf(price)/100,category,type,name,desc,orderStartDate,orderEndDate,validWeeks);
...
//                for(Object ikey : skuJson.keySet()){
                    ...
                    ProductDetail pd = new ProductDetail(Double.valueOf(price),category,type);
                    productMap.get(itemId).addProductDetail(pd);
//                }
                paramMap.clear();
            }
...
//            String ruleUrl = MessageFormat.format(RULE_URL, itemId);
    ...
    public synchronized void execute(TaskExecution te) {
        ...
        Spider.create(this).addUrl(newUrl).thread(5).run();
        logger.info("*** 数据抓取完毕，正在保存中 ***");
        String taskId = taskScheduler.getId();
        String taskExecutionId = te.getId();
    ...
    public static void main(String[] strs){
        ProductDetail pd = new ProductDetail(1.00d);
